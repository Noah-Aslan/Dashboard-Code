...
st.set_page_config(page_title="Workforce ML — Heart Failure",layout="wide")
file = 'Heart_Failure_Patient_Data_12_Months_20_Patients.xlsx'
@st.cache_data
def load_data():
    df_person= pd.read_excel(file, sheet_name='PERSON_MONTH_DATA')
    df_act=pd.read_excel(file, sheet_name='FCT_ACTIVITY_DATA')
    df_act['ACTIVITY_MONTH']=pd.to_datetime(df_act['ACTIVITY_MONTH'])
    df_person['ANALYSIS_MONTH']=pd.to_datetime(df_person['ANALYSIS_MONTH'])
    return df_person, df_act
df_person, df_act=load_data()
st.header("Workforce Planning & Predictions")
st.caption("Based on Jan 2024 – Dec 2024 | 20 heart failure patients | All services")
workforce=df_act.groupby('ACTIVITY_MONTH').agg(
...
for col, (colour, label) in service_cols.items():
...
for col, (colour, label) in hour_cols.items():
...
try:
    ...
    # yearly_seasonality=False — only 1 year of training data
...
# Initialise with safe defaults — used by Step 4 summary panel below
high_risk=0
med_risk=0
low_risk=0
latest_data=None
try:
    ...
    df_act_copy['ACTIVITY_MONTH']=pd.to_datetime(df_act_copy['ACTIVITY_MONTH'])
    df_act_copy['SK_PATIENT_ID_STR']='P' + df_act_copy['SK_PATIENT_ID'].astype(str)
    ...
    feature_cols=['AGE', 'GENDER_ENC', 'IMD_QUINTILE_19', 'TOTAL_ACTIVE_CONDITIONS',
    ...
    # Target: 1 if patient had an emergency admission this month
    ...
    model_xgb.fit(X, y)
    # Score each patient on their most recent month
...
flagged=pd.DataFrame()   # safe default — used by summary panel below
try:
